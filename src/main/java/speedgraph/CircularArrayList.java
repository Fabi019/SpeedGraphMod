...
public class CircularArrayList<E> extends AbstractList<E> implements RandomAccess {
    ...
    public CircularArrayList(int capacity) {
        n = capacity + 1;
        buf = new CopyOnWriteArrayList<>(Collections.nCopies(n, (E) null));
    }
    ...
    public E getOldest() {
        ...
        for (; ; i = wrapIndex(++i)) {
            if (buf.get(i) != null) break;
            if (i == leader)
                return null; //throw new IllegalStateException("Cannot remove element. CircularArrayList is empty.");
        }
    ...
    public E getNewest() {
        int i = wrapIndex(leader - 1);
        if (buf.get(i) == null)
            return null; // throw new IndexOutOfBoundsException("Error while retrieving the newest element. The Circular Array list is empty.");
        return buf.get(i);
    }
